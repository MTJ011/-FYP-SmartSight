...
from models.product_model import get_product_by_barcode, insert_product
...
def run(barcode):
    ...
    name, category, brand, type_ = get_food_product(barcode)

    if not name:
        name, category, brand, type_ = get_beauty_product(barcode)

    if name:
        save_local_price(barcode, name, brand, category, 0)

        result = f"{name}. Category: {category}."
        print(f"✅ Found online: {result}")
        speak(result)  # ✅ Single speak — removed the redundant "I found {name}" before it
        return result
